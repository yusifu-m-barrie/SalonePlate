const API_HOST =
  process.env.EXPO_PUBLIC_API_URL?.replace(/\/api\/v1\/?$/, '') || 'http://localhost:4000';

const PLACEHOLDER =
  'https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=600';

/** Ensure menu/restaurant images load on device (fixes localhost URLs from uploads). */
...
const URL_LIKE_KEYS = /^(imageUrl|coverImage|logoUrl|avatarUrl|galleryUrls|photos|thumbnailUrl)$/i;

/** Rewrite all image fields in API JSON responses automatically. */
  ...
  if (typeof data === 'object') {
    ...
    for (const [key, value] of Object.entries(data as Record<string, unknown>)) {
      if (key === 'galleryUrls' && Array.isArray(value)) {
        out[key] = value.map((v) => (typeof v === 'string' ? resolveImageUrl(v) : rewriteMediaInJson(v)));
